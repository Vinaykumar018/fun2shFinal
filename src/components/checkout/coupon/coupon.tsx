...
const Coupon = ({ subtotal }: { subtotal: number }) => {
  ...
  return (
    <div className="w-full">
      <p
        // role="button"
        className="text-[13px] font-bold mb-4 text-heading transition duration-200 hover:text-accent"
      >
        {t('text-have-coupon')}
      </p>
      <form
      ...
      {errors.code && (
        <p className="text-red-500 mt-2 text-sm">{t(errors.code.message)}</p>
      )}
    </div>
  );
};
